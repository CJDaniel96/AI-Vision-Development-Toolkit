...
def process_video(video_path: str, model, output_folder: str, confidence_threshold: float, frame_interval: int, save_frames: bool, verbose: bool) -> Tuple[int, int]:
    ...
    try:
        ...
        for result in results_generator:
            ...
            try:
                # 處理偵測結果
                # process_yolo_results 預期一個列表，所以我們將單個結果放入列表中
                detections = process_yolo_results([result], confidence_threshold)
                
                # 從結果中取得幀的尺寸和影像
                height, width = result.orig_shape
                depth = result.orig_img.shape[2] if len(result.orig_img.shape) > 2 else 3
